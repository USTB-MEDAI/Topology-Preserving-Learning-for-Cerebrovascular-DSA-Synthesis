    ...
    def validation_step(self, batch, batch_idx):
        ...
        if self.current_epoch % 10 == 0 and batch_idx == 1:
            ...
            if self.current_epoch % 200 == 0 and batch_idx == 1:
                ddim_sampler = DDIMSampler(self)
                shape = (self.batch_size, self.channels, self.image_size, self.image_size)  #mdf shan image_size
...
class DiffusionWrapper(pl.LightningModule):
    ...
    def forward(self, x, t, c_concat: list = None, c_crossattn: list = None):
        if self.conditioning_key is None:
            ...
            # # 确保 c_concat 是 4D 张量   mdf
            # print(f"x shape0 :{x.shape}")
            # print(c_concat[0].shape)
            # if c_concat is not None:
            #     c_concat = [item.flatten(2) if item.dim() == 5 else item for item in c_concat]
            # # if c_concat is not None:
            # #     c_concat = [item.view(item.size(0), item.size(1), item.size(2), item.size(3)) if item.dim() == 5 else item for item in c_concat]
