...
import torch.nn.functional as F  #3.23mdf

from .skeleton_loss import skeleton_loss_batch  # 7.15mdf
...
class DDPM(pl.LightningModule):
    ...
    def get_loss(self, pred, target, mean=True):
        ...
        # skeleton loss  ##mdf
        # elif self.loss_type == "skeleton":
        #     import skeleton_loss
        #     pred_skeleton = extract_skeleton(pred)
        #     target_skeleton = extract_skeleton(target)
        #     loss = (pred_skeleton - target_skeleton).abs().mean()
        elif self.loss_type == "skeleton":
            # 骨架感知损失：通过 Chamfer 距离度量预测图与目标图的骨架相似度
            loss = skeleton_loss_batch(pred, target, threshold=0.15)
            # loss = torch.tensor(loss, device=pred.device)  # 确保返回的是 tensor
            loss = loss.clone().detach().to(pred.device).requires_grad_(True)
...
class LatentDiffusion(DDPM):
    ...
    def instantiate_first_stage(self, config):    #参照这个改
        # model = instantiate_from_config(config)
        model = AutoencoderKL(**config)       #mdf 用ae模型训cond，传入yaml的cond配置
    ...
    def instantiate_cond_stage(self, config):     #改这个，注释掉，替换成自己的
    ...
    def get_learned_conditioning(self, c):
        ...
        return c
    #def condition_jpg_encoder(self, cond)   #改了放到这 替换掉下面vit调用
    ...
    def condition_vit_encode(self, cond):
        ...
        cond = cond.repeat(1, 3, 1, 1)   #这个复制三份不要 
    ...
    def get_input(
        ...
        if "skeleton" in batch.keys():
            x_b = batch["skeleton"].as_tensor()
            encoder_posterior_b = self.encode_first_stage(x_b)
            z_b = self.get_first_stage_encoding(encoder_posterior_b).detach()
        else:
            z_b = None
        ...
        #     out.extend(cond)
        if return_original_cond:
            out.extend([c for c in cond if c is not None])

        # print(f"cond1: {cond1 is not None}, cond2: {cond2 is not None}, cond3: {cond3 is not None}") #mdf
        # print(f"get_input() 返回值数量: {len(out)}")  #mdf
        return out
    ...
    def shared_step(self, batch, **kwargs):
        x, c, x_b= self.get_input(batch, self.first_stage_key)  # * x: fisrt_stage gauss sample  ,c origin image
        loss = self(x, c, x_b)
        return loss, c

    def forward(self, x, c, x_b=None, *args, **kwargs):   #mdf723  ske
        t = torch.randint(0, self.num_timesteps, (x.shape[0],), device=self.device).long()
        ...
        return self.p_losses(x, c, t, x_b, *args, **kwargs)
    ...
    def apply_model(self, x_noisy, t, cond, return_ids=False, type=0):
        ...
        if hasattr(self, "split_input_params"):
            ...
            x_recon = self.model(x_noisy, t, **cond, type=type)
    ...
    def p_losses(self, x_start, cond, t, x_start_b=None, noise=None):
        ...
        model_output = self.apply_model(x_noisy, t, cond, type=0)   #717
        
        bone_flag = x_start_b is not None
        
        if bone_flag:
            x_noisy_b = self.q_sample(x_start=x_start_b, t=t, noise=noise)  # * x -> x_t (noisey)
            model_output_b = self.apply_model(x_noisy_b, t, cond, type=1)   #717
        ...
        # #mdf715
        loss_simple = self.get_loss(model_output, target, mean=False).mean([1, 2, 3])  #mdf , 4    #717
        ...
        # 7.17
        loss_vlb = self.get_loss(model_output, target, mean=False).mean(dim=(1, 2, 3))  #mdf , 4   mdf715
        ...
        if bone_flag:
            # 计算骨架分支的简单损失
            loss_simple_b = self.get_loss(model_output_b, target_b, mean=False).mean([1, 2, 3])
            loss_dict.update({f"{prefix}/loss_simple_b": loss_simple_b.mean()})

            # 骨架分支的带logvar的损失（和主分支共享logvar）
            loss_b = loss_simple_b / torch.exp(logvar_t) + logvar_t
            loss_b = self.l_simple_weight * loss_b.mean()  # 可单独设置骨架权重

            # 总损失 = 主损失 + 骨架损失（加权）
            total_loss = loss + self.lambda_skel * loss_b
            loss_dict.update({f"{prefix}/loss_skeleton": loss_b.mean()})
    ...
    def p_losses_origin(self, x_start, cond, t, noise=None):   #原loss
        noise = default(noise, lambda: torch.randn_like(x_start))  # * [64,64,4,4,4]
        x_noisy = self.q_sample(x_start=x_start, t=t, noise=noise)  # * x -> x_t (noisey)
        model_output = self.apply_model(x_noisy, t, cond)   #717
        ...
        loss_simple = self.get_loss(model_output, target, mean=False).mean([1, 2, 3])  #mdf , 4    #717
        # #7.17
        # loss_simple_raw = self.get_loss(model_output, target, mean=False)
        # if self.loss_type == "skeleton":
        #     # loss_simple_raw 是形状 [B] 的 Tensor（每个样本一个 loss），保持不变即可
        #     loss_simple = loss_simple_raw.to(model_output.device)
        # else:
        #     # 非 skeleton loss：返回的是 shape [B, C, H, W]，按维度求 mean
        #     loss_simple = loss_simple_raw.mean(dim=(1, 2, 3))
        ...
        # if self.loss_type == "skeleton":
        #     # loss_simple_raw 是标量 Tensor，复制成 batch 维
        #     loss_simple = loss_simple_raw.clone().detach().to(model_output.device).expand(model_output.size(0))
        # else:
        #     # 非 skeleton loss：返回的是 shape [B, C, H, W]，按维度求 mean
        #     loss_simple = loss_simple_raw.mean(dim=(1, 2, 3))
        ...
        # if self.loss_type == "skeleton":
        #     # 返回的是标量，不需要 mean()
        #     loss_simple = torch.tensor(loss_simple_raw).to(model_output.device).repeat(model_output.size(0))
        # else:
        #     loss_simple = loss_simple_raw.mean([1, 2, 3])
        ...
        # if self.loss_type == "skeleton":
        #     # Chamfer loss（骨架正则），loss_simple 已是 [B] Tensor，求 batch mean
        #     loss = self.l_simple_weight * loss_simple.mean()

        #     # loss_vlb 设为 zero tensor，且确保是可导（带 grad_fn）
        #     loss_vlb = torch.zeros(1, device=model_output.device, dtype=loss.dtype, requires_grad=True)
        #     loss_dict.update({f"{prefix}/loss_vlb": loss_vlb})
        # else:
        #     # 默认 VLB loss 计算方式
        #     loss = self.l_simple_weight * loss_simple.mean()
        ...
        # #     # skeleton loss 不计算 loss_vlb，可以设为 0
        # #     loss_vlb = torch.tensor(0.0, device=model_output.device)
        # #     loss_dict.update({f"{prefix}/loss_vlb": loss_vlb})
        # # else:
        # #     loss = self.l_simple_weight * loss_simple.mean()
    ...
    def validation_step(self, batch, batch_idx):
        ...
        if batch_idx == 1:  #self.current_epoch % 10 == 0 and 
            ...
            #     ddim_sampler = DDIMSampler(self)
            #     shape = (self.batch_size, self.channels, self.image_size, self.image_size)  #mdf , self.image_size
            ...
            #     print(f"Before squeeze and permute: reconstructions shape: {reconstructions.shape}, dtype: {reconstructions.dtype}")
            #     # reconstructions = reconstructions.permute( 1,0,2,3) 
            #     reconstructions = reconstructions.squeeze(0).permute( 1, 0, 2)   #mdfpermute(1, 0, 2, 3) （squeeze(0)：移除维度为 0 的维度。通常是批量维度，维度只剩3）
            #     # reconstructions = reconstructions.squeeze(0)
            #     # reconstructions = reconstructions.squeeze(0)
            #     print(f"After squeeze and permute: reconstructions shape: {reconstructions.shape}, dtype: {reconstructions.dtype}")
            #     reconstructions = reconstructions.type(torch.uint8)
            #     grid = make_grid(reconstructions)
            #     print(f"Grid dtype: {grid.dtype}")  # 打印数据类型  mdf
            #     print(f"Grid shape: {grid.shape}, dtype: {grid.dtype}")  #mdf
            ...
            #     x = (x + 1) * 127.5
            #     x = x.squeeze(0).permute(1, 0, 2)   #mdf  , 3 
            #     x = x.type(torch.uint8)
            #     grid = make_grid(x)
            #     self.logger.experiment.add_image("val_gt", grid, self.global_step)

    def img_saver(self, img, post_fix, i_type=".jpg", meta_data=None, **kwargs):
        """
        ...
        assert post_fix in ["origin_x", "ae_rec", "xray_0", "xray_1", "rec"], "unsupported post_fix"  #mdf  "xray_left", "xray_right",
    ...
    def test_step(self, batch, batch_idx):
        ...
        # )
        _, c, _, x, x_rec, origin_cond = self.get_input(
            batch, self.first_stage_key, return_first_stage_outputs=True, return_original_cond=True
        )
        ...
        #     batch, self.first_stage_key, return_first_stage_outputs=True, return_original_cond=True
        # )
        # print("DEBUG: get_input() returned", len(outputs), "items")
        # print("Returned items:", [type(i) for i in outputs])  # 查看类型更安全
...
class DiffusionWrapper(pl.LightningModule):
    def __init__(self, diff_model_config, conditioning_key):
        ...
        self.diffusion_model = UNetModel_sk(**diff_model_config)    #mdf
        self.conditioning_key = conditioning_key
        assert self.conditioning_key in [None, "concat", "crossattn", "hybrid", "adm"]

    def forward(self, x, t, c_concat: list = None, c_crossattn: list = None, type=0):
        if self.conditioning_key is None:
            out = self.diffusion_model(x, t)
        elif self.conditioning_key == "concat":
            # print(f"x shape :{x.shape}") #
            # print(c_concat[0].shape)
            # print(f"1:x shape: {x.shape}")  #mdf
            # for i, c in enumerate(c_concat):
            #     print(f"c_concat[{i}] shape: {c.shape}")
            
            D = c_concat[0].shape[2]  # 获取第三维的大小
            c_concat = [c[:, :, D//2, :, :] for c in c_concat]  # 或者用 mean(dim=2)

            # print(f"2:x shape: {x.shape}")  #mdf
            # for i, c in enumerate(c_concat):
            #     print(f"c_concat[{i}] shape: {c.shape}")
            #c_concat = [c.squeeze(2) if c.dim() == 5 else c for c in c_concat]

            c_concat = [F.interpolate(c, size=(32, 32), mode='bilinear', align_corners=False) for c in c_concat]  #mdf to256
            
            # c_concat = torch.stack(c_concat, dim=0)  #转换为张量
            # c_concat = F.interpolate(c_concat, size=(32, 32), mode='bilinear', align_corners=False)  # 上采样
            # print(type(c_concat))
            # print(f"3:x shape: {x.shape}")  #mdf
            # for i, c in enumerate(c_concat):
            #     print(f"c_concat[{i}] shape: {c.shape}")

           
            # print(f"x shape: {x.shape}")  #mdf
           
            # c_concat = c_concat.squeeze(4)  # 去掉深度维度，变为 [1, 8, 16, 16]
            # c_concat = F.interpolate(c_concat, size=(32, 32), mode='bilinear', align_corners=False)
            
            xc = torch.cat([x] + c_concat, dim=1)

            out = self.diffusion_model(xc, t, type=type)
