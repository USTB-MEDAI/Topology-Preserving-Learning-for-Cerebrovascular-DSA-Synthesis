...
class Upsample(nn.Module):
    ...
    def forward(self, x):
        x = torch.nn.functional.interpolate(x, (x.shape[2] * 2, x.shape[3] * 2), mode="nearest")    #  mdf , x.shape[4] * 2
        if self.with_conv:
            x = self.conv(x)
        return x
...
class Downsample(nn.Module):
    def __init__(self, in_channels, with_conv):
        ...
        if self.with_conv:
            ...
            self.conv = torch.nn.Conv2d(in_channels, in_channels, kernel_size=3, stride=(2, 2), padding=1)   #  修改 stride=(2, 2, 2)
