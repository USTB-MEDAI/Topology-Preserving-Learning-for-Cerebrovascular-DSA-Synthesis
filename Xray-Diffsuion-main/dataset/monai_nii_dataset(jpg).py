...
from monai.data import NibabelReader #122ssy
...
def prepare_dataset(data_path, resize_size, img_resize_size=None, cond_path=None, split="train",cond_nums=[1,2]):
    ...
    data_list = sorted(Path(data_path).glob("*.jpg"))                               # modify    *.nii*
    if cond_path:
...
def test_save_image():
    ...
    #/disk/ssy/data/DIAS/myDIAS/training/labelsjpg/label_s0.jpg
    trans = Compose(
        [
            ##LoadImaged(keys="image", ensure_channel_first=True),           ##  modify  122ssy
            LoadImaged(keys=["image"], reader=NibabelReader()), 
            # Spacingd(keys=["image"], pixdim=(1.0, 1.0, 1.0), mode=("bilinear")),
            Resized(keys="image", spatial_size=(128, 128)),             # modify  spatial_size=(128, 128, 128)
    ...
    # print(img.meta.keys())
    img = (img + 1) * 127.5
    img = img.squeeze()  # 去除额外的维度
    img = img.numpy().astype(np.uint8)  # 转换为 numpy 数组，并且确保数据类型为 uint8
    print(img.shape)
    print("2")
    saver_origin = SaveImage(
        output_dir="./",
        # output_ext=".nii.gz",                 #modify
        output_ext=".jpg",
        ...
        squeeze_end_dims=True,
        # writer="NibabelWriter",
        writer="PILWriter",               #PILWriter  modify
    )
    saver_origin(img)
